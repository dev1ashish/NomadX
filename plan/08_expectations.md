...
## 2026-05-17 — Band-chemistry Stage 1: bacteria-only ANOVA + primary triple

Pre-registration for [plan/14 §6.2 + §6.3](14_band_chemistry_research.md).
Two research questions resolved in one script run; predictions locked
**before** the analysis runs.

### RQ4 — Bacteria-only ANOVA on preprocessed spectra (H₂O excluded)

Re-run per-bin ANOVA-F on the 3 bacterial classes only.

| Quantity | Predicted | Actual | Verdict |
|---|---|---|---|
| Top-30 bins concentrate in fingerprint region (1100–1750 cm⁻¹) | yes (≥20 of 30) | — | pending |
| At least 2 of {1338, 1454, 1658} appear in top-30 | yes | — | pending |
| All 3 of {1338, 1454, 1658} appear in top-30 | possibly (50/50) | — | pending |
| Top region centroid shifts away from 2880–2940 C-H stretch | yes (≤5 of 30 in C-H region) | — | pending |
| LPS region (800–1200) contributes top-30 bins | yes (≥5 bins) | — | pending |

**Reasoning.** The 4-class ANOVA top-30 was *all* in 2880–2940 cm⁻¹ C-H
stretch ([07§anova-bins-vs-stec-discriminative-bands](07_findings.md#2026-05-14--anova-bins-vs-stec-discriminative-bands))
because that's the largest water-vs-bacteria difference. Removing H₂O
should demote those bins and surface within-bacterial chemistry. Per
...
the load-bearing STEC bands are 1338/1454/1658.

### RQ1 — STEC vs Non-STEC effect sizes at the primary triple

For each of 1338, 1454, 1658: integrate ±10 cm⁻¹ AUC per spectrum, test
STEC (n_files=27) vs Non-STEC (n_files=25). Both spectrum-level (n inflated
by within-file similarity 0.997) and **file-level** (true independence)
statistics reported.

| Band | Predicted d (file-level) | File-level Welch t p | Actual d (file) | Actual p (file) | Verdict |
|---|---|---|---|---|---|
| 1338 (NA, stx) | 0.30 – 0.80 | < 0.05 | — | — | pending |
| 1454 (lipid)   | 0.20 – 0.60 | < 0.10 | — | — | pending |
| 1658 (amide-I) | 0.30 – 0.80 | < 0.05 | — | — | pending |

Also reported: spectrum-level d (informational only — n=4600+ gives
significance regardless), Mann-Whitney U, sign of the shift per band,
per-strain breakdown (O157 vs O121 vs O103 vs K-12 vs ATCC25922 vs 83972),
and best-single-band AUROC.

**Reasoning.** File-level n ≈ 25 per side; real power needs d ≥ 0.5. The
spectrum-level test is foregone-significant given n inflation, so the
**file-level p is the evidence bar**. Cohen's d prior comes from `cisek-2013`
showing >95% pure-culture discrimination (which back-calculates to d > 1.0
under their conditions); we'd expect smaller effects on bacteria-on-substrate.
A null at all 3 bands (file-level p > 0.1, d < 0.1) would imply published
bands aren't load-bearing on this dataset — the most interesting finding
either way.
...
Predicted: at all 3 bands, the STEC vs Non-STEC mean shift is driven by
**O157 + O121 + O103 collectively** (all 3 STEC strains contribute), not
by O157 alone. If only O157 shifts, the briefing's "STEC vs Non-STEC"
framing is really "O157 vs the rest" — narrower than implied.

### Post-run resolution (2026-05-17)

**Branch (C) hit.** Full writeup at [07§2026-05-17-stec-triple-does-not-replicate](07_findings.md#2026-05-17--stec-triple-does-not-replicate).
...
1. **Stage 2 (band_features module) anchor changes.** Headline features = the LPS region 800–1200 integrated AUC + narrow windows at 1050 / 1117 / 1194 (the actual top discriminators). Literature triple kept as supporting features for the negative-finding writeup.
...
## 2026-05-17 — Band-chemistry Stage 2: `atlas/band_features.py` module

Pre-registration for [plan/14 §5](14_band_chemistry_research.md). Stage 2 is
mostly an infrastructure stage — the module + cached `feature_frame.parquet`
— but it ships with smoke checks whose results are pre-registered here.

### Smoke check predictions

| Check | Predicted | Actual | Verdict |
|---|---|---:|:-:|
| Macromolecule LPS AUC differs E. coli vs Salmonella by ≥ 0.5 SD (file-level) | yes | — | pending |
| Macromolecule NA / amide / aromatic-AA group AUC differs STEC vs Non-STEC by < 0.3 SD (file-level) — consistent with Stage 1 Branch (C) | yes (all 3 below 0.3) | — | pending |
| New empirical-anchor 800–1200 LPS AUC: STEC vs Non-STEC file-level Cohen's d ≥ 0.4 | yes (Stage 1 showed top discriminators at 1117 and 1194) | — | pending |
| Lorentzian fit success rate, per-band, per-spectrum (defined as fit returns finite center within ±20 cm⁻¹ of catalog center and FWHM in 5–40 cm⁻¹) | ≥ 80% on most bands; literature triple may underperform (catch the peak-position-drift hypothesis) | — | pending |
| Fitted peak-center mean within ±3 cm⁻¹ of catalog center for clear single-peak bands (e.g. 1050, 1117, 1194) | yes | — | pending |
| Fitted peak-center for 1454 — possible drift since the band sits in an overlapping CH₂ deformation cluster | catalog 1454, fitted likely 1448–1456 | — | pending |
| Feature cache size | 7,122 rows × 25–40 columns | — | pending |
| Macromolecule AUC for H₂O class at every group | all groups significantly lower than any bacterial class (water has no biological vibrations) | — | pending |

### Pre-committed module surface (no test of internals — that's unit-test
territory, not pre-reg territory)

`integrate_band`, `integrate_region`, `macromolecule_vector`, `band_ratios`,
`fit_peak`, `fit_peaks_batch`, `feature_frame`, `BANDS`, `MACROMOLECULE_GROUPS`.
All defined in plan/14 §5.1.

### Reasoning

The Stage 1 finding ([07§stec-triple-does-not-replicate](07_findings.md#2026-05-17--stec-triple-does-not-replicate))
sets all of these. The LPS-vs-non-LPS asymmetry is the headline biology
prediction; if even the macromolecule radar can't surface the E. coli vs
Salmonella LPS difference, the module is broken. If the literature triple
bands fit cleanly with peak centers right at 1338/1454/1658, the "peak-shift
hypothesis" for Stage 1's null gets falsified — meaning the bands genuinely
don't carry STEC-vs-Non-STEC signal on this dataset (rather than carrying
signal at a slightly different wavenumber the ±10 window missed).

### Stage-gate

If LPS asymmetry doesn't show up at all (file-level Cohen's d < 0.3 for
E. coli vs Salmonella on LPS group AUC), **pause and debug the module before
Stage 3.** Almost-certainly a bug — the per-file mean spectra in
`images/_summary/07_annotated_preprocessed_spectra.png` show the LPS region
visibly differing across primary classes.

### Post-run resolution (2026-05-17)

**Stage-gate: PASSED.** LPS asymmetry E. coli vs Salmonella file-level
Cohen's d = **−0.634** (`auc_lps_chain_discrim`, Salmonella > E. coli) — well
above the 0.3 gate. Stage 3 launchable.

Detailed verdicts table at [07§2026-05-17-stage2-empirical-anchors-and-amide-shift](07_findings.md#2026-05-17--stage2-empirical-anchors-and-amide-shift).
Summary:

| Prediction | Verdict | Notes |
|---|:-:|---|
| LPS E. coli vs Salmonella \|d\| ≥ 0.5 | ✅ | d = −0.63 |
| Macro literature groups STEC vs Non-STEC \|d\| < 0.3 | ⚠️ 4/5 | metabolite at −0.41; aromatic/amide/NA/lipid all in range |
| 800–1200 LPS region STEC vs Non-STEC \|d\| ≥ 0.4 | ⚠️ | broad d = 0.28 (below floor), **but narrow lps_1117 = 0.77, lps_1194 = 1.03** (massive overshoots) |
| Lorentzian fit success ≥ 80% | ❌ | 0.2–37% across 8 bands. **Stage 4 gated on a fit-method fix.** |
| Fit center within ±3 cm⁻¹ catalog | ✅ partial | most bands; amide_i_1658 drifts +1.3 to +4.1 (real biology, kept as a finding) |
| 1454 fit drift in 1448–1456 range | ✅ | STEC/NSTEC/Salmonella all within 1 cm⁻¹ of 1454 |
| Feature cache 25–40 columns | ⚠️ | 81 (kept) |
| H₂O < bacterial on every macromolecule group | ⚠️ 2/4 | protein/lipid yes; aromatic_aa / NA unexpectedly H₂O > bacterial (post-SNV substrate residual hypothesis) |

**Headline finding from Stage 2.** `auc_lps_1194` file-level d = +1.03
(STEC > Non-STEC). Cleanest single-band STEC↔Non-STEC discriminator in the
whole project. Together with `auc_lps_1117` (d = +0.77), these two empirical
bands in the user-highlighted 800–1200 LPS chain region carry the actual
within-E. coli signal at sub-bin specificity.

**Pre-registration miscalibration to log.** The broad-region prediction
("800–1200 integrated AUC d ≥ 0.4") was wrong — broad integration dilutes
the per-bin signal. The narrow ±10 windows at the empirical-anchor centers
are the right operationalization. **For Stage 5 the classifier should use
the narrow windows as features, not the broad-region AUC.**
...
2. **Stage 4 gated on a Lorentzian-fit fix.** Three repair options to try in
   order: (a) linear baseline instead of flat; (b) loosen FWHM bounds to [3, 60];
   (c) fit only per-file-mean spectra (87 fits, much cleaner signal).
3. **Stage 5 anchor feature set locked.** Headline features = `auc_lps_chain_discrim`,
   `auc_lps_1050`, `auc_lps_1117`, `auc_lps_1194`, plus the 10 default ratios.
   Literature-triple AUCs included as supporting features.
4. **Active-stage memory updated** ([[atlas-band-chemistry-roadmap]]): Stage 2 → done; Stage 3 anchor set.

---

## 2026-05-17 — Band-chemistry Stage 3: `notebooks/band_chemistry.ipynb` sections B–E

Pre-registration for [plan/14 §6.2–6.5](14_band_chemistry_research.md).
Stage 3 is visualization-only — no new modeling; reads the
`data_cache/band_features.parquet` cache from Stage 2 and renders the
chemistry-aware story.

### Locked-in scope decisions

- **Radar normalization scheme:** z-score each macromolecule axis across all
  classes (so axes are comparable in units of "SD from grand mean"). Reason:
  raw SNV-unit AUCs are negative numbers around −20 to −60 — radar plots with
  negative radii don't render correctly, and the cross-class *relative* shape
  is what matters, not absolute values. Per-class-mean offset would lose the
  scale information; z-score keeps both.
- **Subclass overlay scope:** one primary-class radar (4-way overlay), plus
  three subclass radars (one panel per parent — STEC strains, Non-STEC strains,
  Salmonella strains). H₂O on the primary radar only.
- **Ratio scatters to ship:** (a) `lps_1194 / lps_1050` × `lps_1117 / lps_1050`
  (the two big-d empirical anchors normalized by the cross-class baseline);
  (b) `amide_1658 / na_1338` × `lps_chain_over_protein` (literature triple
  ratio vs new empirical ratio, on the same axes); (c) macromolecule
  group-vector scatter of `auc_protein_amide` × `auc_lipid_carbohydrate` —
  Cisek's two largest groups, file-level.

### Predictions

| Quantity | Predicted | Actual | Verdict |
|---|---|---|---|
| Radar: Salmonella's LPS axis (chain-discrim region) is ≥1 z-SD above the E. coli classes' LPS axis | yes | — | pending |
| Radar: STEC and Non-STEC overlap visually on protein_amide, nucleic_acid, aromatic_aa axes (no clean separation in the 5 catalog macromolecule groups — consistent with Stage 1/2 nulls) | yes | — | pending |
| Best-1D ratio AUROC for STEC vs Non-STEC (file-level), top ratio | ≥ 0.80 (anchored on Stage 2's lps_1194 d=+1.03 → AUROC ≈ 0.77, but ratios may improve it) | — | pending |
| Best-1D ratio for STEC vs Non-STEC contains lps_1117 OR lps_1194 in numerator/denominator | yes | — | pending |
| Best-1D ratio AUROC for E. coli vs Salmonella (file-level), top ratio | ≥ 0.75 (anchored on Stage 2 lps_chain_discrim d=−0.63 → AUROC ≈ 0.67, ratios should improve) | — | pending |
| `amide_1658 / na_1338` (Cisek-style ratio) AUROC for STEC vs Non-STEC | < 0.65 (literature bands null at file-level, ratios won't rescue) | — | pending |
| Subclass radars: O157H7 visually separates from O121H19 + O103H2 on at least one axis | possibly (Cisek-2013 tested O157H7 specifically — if anything in literature replicates, O157H7-specific) | — | pending |

### Reasoning

The dominant Stage 2 finding was that single empirical bands (1117, 1194)
carry strong file-level signal while macromolecule-group aggregation doesn't —
implying the discriminative signal is *narrow-spectral* not *broad-chemical*.
Ratios should preserve the narrow-band advantage where they're built around
1117/1194; literature-style ratios should remain null. The radar visualization
is the *interpretability* deliverable, not the *discrimination* deliverable —
it shows the user, in macromolecule vocabulary, *why* the classifier will lean
on the empirical anchors rather than the literature catalog.

### Stage-gate

Stage 5 is the next concrete experiment that needs Stage 3 output. If the
ratio AUROC headlines come in below 0.65 for STEC vs Non-STEC even with the
empirical anchors, **the engineered-feature classifier in Stage 5 should not
be expected to clear LOSO 0.55** (its pre-committed success bar). In that case
Stage 5 still ships as a negative-finding writeup, but the headline pivots to
"per-band AUC + raw spectrum is the right input to a CNN, not engineered ratios."

### Post-run resolution (2026-05-17)

Stage 3 done. Detailed writeup at [07§stage3-radars-ratios-auroc](07_findings.md#2026-05-17--stage3-radars-ratios-auroc). Summary:

| Prediction | Verdict |
|---|:-:|
| Salmonella LPS axis ≥1 σ above E. coli on LPS chain region | ✅ ~1σ gap on 1050; 0.7σ on lps_chain |
| STEC and Non-STEC overlap on protein/NA/AA radar axes | ✅ near-identical pentagons except lipid axis |
| Best-1D ratio AUROC STEC vs Non-STEC ≥ 0.80 | ❌ best ratio = `lipid_over_protein` = 0.741 |
| Best ratio for STEC vs Non-STEC contains lps_1117/1194 | ❌ top ratio has no LPS bands; **lipid_over_protein wins (literature direction validated even though specific bands were wrong)** |
| Best-1D ratio AUROC E. coli vs Salmonella ≥ 0.75 | ✅ marginal — `lps_1117_over_1050` = 0.769 |
| Cisek ratio `amide/NA` AUROC < 0.65 | ✅ not in top-15 (< 0.578) |
| Subclass radars: O157H7 separates from O121/O103 | ⚠️ partial — STEC strains tight; **K-12 separates from ATCC/83972 more than any STEC sibling** (consistent with `soupene-2003-k12` reference) |

**Pre-reg miss to log.** The "ratios outperform" prediction was rooted in
batch-effect-cancellation intuition. On this dataset, the file-level n≈25
per class means within-file averaging already denoises per-spectrum noise; the
discriminative signal is *narrow-spectral*, and any denominator dilutes it.
**Single-band narrow-AUC > ratio at file level.** Stage 5 headline feature
set uses raw `lps_1050/1117/1194` AUCs (not ratios) as anchors.

**Headline finding.** Best 1-D AUROC at file-level:
- STEC vs Non-STEC: `auc_lps_1194` = **0.775** (single-band)
- E. coli vs Salmonella: `auc_lps_1050` = **0.817** (single-band)

**Side finding — literature direction partially validated.** `lipid_over_protein`
ratio AUROC = 0.741 for STEC vs Non-STEC, and `lipid_1454` single-band d=−0.47.
Cisek's "lipid carries STEC signal" claim survives at the **macromolecule
category** level even though the specific band centers don't replicate. The
writeup story sharpens from "literature triple doesn't replicate" to "the
chemistry direction was right; the wavenumber centers were wrong on this
instrument."

**Operational decisions.**
1. **Stage 5 anchor features locked:** the 12-feature set in [07§stage3](07_findings.md#2026-05-17--stage3-radars-ratios-auroc) "Implications for Stage 5". Pre-committed bar: LOSO mean parent-recall ≥ 0.55.
...
## 2026-05-17 — Band-chemistry Stage 5: engineered-feature classifier

Pre-registration for [plan/14 §6.7](14_band_chemistry_research.md), the RQ5
experiment. **Notebook:** `notebooks/feature_engineering.ipynb`. Reads
`data_cache/band_features.parquet` + uses the existing
`data_cache/splits/protocol_{a,b}.json` fold structure.

### Feature set (locked from Stage 3 [07§stage3](07_findings.md#2026-05-17--stage3-radars-ratios-auroc))
...
- **XGBoost** — chosen because (a) Stage 2 d=+1.03 at lps_1194 means the discriminative signal is in one feature; trees can split on this cleanly. (b) Existing XGB hyperparameter regime (n_estimators 100–300, max_depth 3–6) works well on this dataset per [10§xgboost-spec](10_decision_log.md).
...
| Quantity | Predicted | Rationale | Actual | Verdict |
|---|---|---|---:|:-:|
| Protocol A file-macro-F1 (XGBoost, mean over 5 folds) | 0.70 – 0.85 | Stage 3 file-level AUROC was 0.77 at the best single band; trees should clear that with engineered features | — | pending |
| Protocol A file-macro-F1 (LogReg) | 0.55 – 0.75 | Linear can exploit the single strong feature but won't combine non-linearly | — | pending |
| Protocol A file-macro-F1 (XGB + LogReg soft-vote) | 0.65 – 0.85 | Roughly the XGB number; ensembles usually don't beat the strong member by much | — | pending |
| **LOSO mean parent-recall (XGBoost) — THE GATE** | 0.45 – 0.60 (success bar = ≥0.55) | The discriminative signal IS in 1194 with file-level d=1.03, so cross-strain transfer of the chemistry should hold partially. PLS-DA's 0.603 on raw 987 bins remains the ceiling — engineered features have ~75× less input information but more concentrated signal | — | pending |
| LOSO mean parent-recall (LogReg) | 0.35 – 0.55 | Linear baseline; should beat vanilla CNN (0.35) but trail XGB | — | pending |
| LOSO mean parent-recall (XGB + LogReg soft-vote) | 0.45 – 0.60 | Same as best solo (XGB) | — | pending |
| O157H7 LOSO recall (XGB) | 0.30 – 0.65 | Stage 3 best discriminator lps_1194 has STEC > Non-STEC sign, O157H7 was *lowest* in STEC by per-strain breakdown → mid-range outcome | — | pending |
| O121H19 LOSO recall (XGB) | 0.40 – 0.85 | O121H19 has intermediate lps_1194; comparable to PLS-DA's 0.89 (best record) is plausible | — | pending |
| O103H2 LOSO recall (XGB) | 0.40 – 0.80 | O103H2 has *highest* STEC lps_1194 → easiest STEC strain for this feature set | — | pending |
| K-12 LOSO recall (XGB) | 0.00 – 0.40 | K-12 is laboratory-domesticated ([`soupene-2003-k12`](11_references.md#soupene-2003-k12--soupene-et-al-j-bacteriol-2003-laboratory-strains-of-escherichia-coli-k-12-things-are-seldom-what-they-seem)); fragile across all models (DANN λ=0.3 reached 0.75 only on 2/5 seeds) | — | pending |
| ATCC25922 LOSO recall (XGB) | 0.40 – 0.85 | ATCC25922 is the canonical lab Non-STEC; ATCC-specific features should be available in literature-band AUCs | — | pending |
| 83972 LOSO recall (XGB) | 0.30 – 0.75 | Commensal strain; PLS-DA = 0.875 on this, hard ceiling | — | pending |
| Salmonella (any of 3 strains) LOSO recall (XGB), highest of the trio | 0.40 – 0.90 | E. coli vs Salmonella has AUROC 0.817 at 1050 — strongest gap in dataset | — | pending |

### Branching verdicts

- **(A) LOSO mean ≥ 0.55 (interpretability parity).** Engineered features clear
  the pre-committed bar. Stage 5 ships as a headline alongside PLS-DA solo;
  per-strain table adds engineered-feature model as a new arm. Writeup leads
  with "you can do this with 13 features instead of 987 raw bins."
- **(B) 0.45 ≤ LOSO mean < 0.55.** Below bar but well above vanilla CNN (0.35).
  Documented as "engineered features capture most of the chemistry signal but
  not enough for cross-strain LOSO parity with PLS-DA." Still publishable as
  the *interpretable* version of "what the model sees."
- **(C) LOSO mean < 0.45.** Engineered features fail to generalize to new
  strains. **Pivot story:** the file-level signal at lps_1194 (d=+1.03) is
  real, but it's **strain-specific** — O103H2's 1194 AUC distribution sits in
  a different place than the held-out strain's 1194 AUC distribution. The
  feature engineering succeeds at "describing the data" and fails at
  "extrapolating across strains" — that's the LOSO crater all our models
  hit, expressed through interpretable features instead of opaque CNN
  weights.

### Stage-gate

If verdict (C), **do NOT proceed to Stage 6 (3-channel CNN with band features)** —
adding the same non-generalizing features as an extra channel will not fix the
generalization problem. Skip directly to Stage 7 (mixed-sample simulation) and
Stage 8 (MCR-ALS cross-validation).

### Post-run resolution (2026-05-17)

**Branch (C) hit.** Best LOSO mean = **0.312** (XGBoost), below the 0.45
floor. Full writeup: [07§stage5-band-classifier](07_findings.md#2026-05-17--stage5-band-classifier).

| Quantity | Predicted | Actual | Verdict |
|---|---|---:|:-:|
| Protocol A XGB file-macro-F1 | 0.70–0.85 | **0.870 ± 0.120** | ✅ slightly above |
| Protocol A LogReg file-macro-F1 | 0.55–0.75 | 0.679 | ✅ |
| Protocol A Ensemble file-macro-F1 | 0.65–0.85 | 0.836 | ✅ |
| LOSO mean XGB (gate ≥0.55) | 0.45–0.60 | **0.312** | ❌ Branch (C) |
| LOSO mean LogReg | 0.35–0.55 | **0.074** | ❌ way below even vanilla CNN |
| LOSO mean Ensemble | 0.45–0.60 | 0.173 | ❌ |
| O157H7 LOSO XGB | 0.30–0.65 | 0.00 | ❌ |
| **O121H19 LOSO XGB** | **0.40–0.85** | **0.89 ⭐** | ⭐ above ceiling — TIES PLS-DA project record |
| O103H2 LOSO XGB | 0.40–0.80 | 0.67 | ✅ |
| K-12 LOSO XGB | 0.00–0.40 | 0.00 | ✅ at floor |
| ATCC25922 LOSO XGB | 0.40–0.85 | 0.11 | ❌ |
| 83972 LOSO XGB | 0.30–0.75 | 0.25 | ❌ |
| Best Salmonella LOSO XGB | 0.40–0.90 | 0.33 (Typhimurium) | ❌ |

**Two pre-reg outcomes worth highlighting:**

1. **The LOSO failure is asymmetric across strains.** Band XGB ties PLS-DA's
   project record on O121H19 (0.89) and reaches 0.67 on O103H2 — both
   non-O157 STEC strains. Same architecture catastrophically fails on
   O157H7 (0.00), ATCC25922 (0.11), and the entire Salmonella triplet (≤0.33).
   Mechanism: the LPS chain region carries serogroup-specific information that
   transfers between similarly-structured non-O157 STECs but not between
   structurally-distinct O-antigens. Consistent with `non-o157-stec-overview`.

2. **Protocol A 0.870 is the second-highest non-PLS-DA file-macro-F1 in the
   project**, behind PLS-DA solo's 0.951. With only 13 features (vs PLS-DA's
   987 raw bins). **The interpretability story holds within-distribution.**

**Operational decisions.**

1. **Stage 6 SKIPPED per stage-gate.** 3-channel CNN with band features won't
   fix the strain-specificity problem. Budget saved: 0.5d.
2. **Per-strain best-model table updated.** Band XGB joint-best on O121H19 with
   PLS-DA (both at 0.89); 3rd-best on O103H2.
3. **Memory + roadmap updated:** Stage 5 → done with Branch (C); Stage 6 → skipped;
   Stage 7 (mixed-sample simulation) next.
4. **Plan/14 §6.8 (Stage 6 / 3-channel CNN) marked SKIPPED** with link to this
   post-run resolution.
5. **Writeup framing locked.** The headline becomes "13 chemistry-grounded
   features match PLS-DA within-distribution at file F1 = 0.87 and tie
   PLS-DA's record on O121H19 LOSO, but the LPS-anchored features carry
   serogroup-specific information that doesn't extrapolate to held-out
   strains with structurally-distinct O-antigens." Negative finding +
   per-strain positive findings, no overclaim.

---

## 2026-05-18 — Band-chemistry Stage 7: mixed-sample degradation simulation

Pre-registration for [plan/14 §"Future" Mixed-sample simulation](14_band_chemistry_research.md),
promoted to Stage 7 per the Stage 5 stage-gate decision. Tests the briefing's
([[atlas-briefing-emphasis]] §3) "10–20% drop vs pure-culture" claim against
our Stage-5 XGBoost classifier.

### Method

- Train Stage 5 XGBoost on the band-feature cache (the 13-feature anchor set
  from Stage 5), using all pure-culture data (no fold structure — this is a
  within-distribution test, not LOSO).
- For each of three pairwise class mixtures — **STEC × Non-STEC**,
  **STEC × Salmonella**, **Non-STEC × Salmonella** — synthesize mixtures by
  linearly combining per-file mean preprocessed spectra at α ∈ {0.0, 0.1,
  0.2, ..., 1.0}.
- For each mixture spectrum, compute the same 13 band features and predict
  the class with the trained XGB.
- Define **success** as: the predicted class matches the majority component
  (whichever class has α > 0.5). At α = 0.5 the chance baseline is ~0.5
  (binary).
...
### Post-run resolution (2026-05-18)

**Branch (B)** — 2/3 pairs validate the briefing at α=0.7. Detailed writeup
at [07§stage7-mixed-sample](07_findings.md#2026-05-18--stage7-mixed-sample).

| Pair | Predicted (α=0.7) | Actual | Verdict |
|---|---|---:|:-:|
| STEC × Non-STEC       | 0.55–0.70 | 0.738 | ✅ |
| STEC × Salmonella     | 0.70–0.85 | 0.582 | ⚠️ below floor but in briefing's 0.55–0.75 range |
| Non-STEC × Salmonella | 0.70–0.85 | **0.279** | ❌ collapse |

**Big methodological finding:** the α=1.0 baseline (pure single-class file mean)
classifies at 0.24 / 0.67 / 0.67 — much lower than the Protocol-A file-F1 0.87.
**Per-file-mean classification ≠ per-pixel + soft-vote pipeline.** The Stage 5
classifier ships ONLY if the deployment pipeline aggregates per-pixel
predictions, not per-pixel-averaged spectra.

**Second finding:** classifier has a **STEC-default class bias** under
uncertainty. At α=0.5 across all 3 mixtures, STEC is the most-predicted class
(44–72%), even when neither input is STEC. The 13-feature anchor space puts
Non-STEC files in a low-density region, so any feature shift moves them
toward STEC. Plan/15 (new feature-engineering research) should explicitly
broaden Non-STEC representation.

**Operational decisions.**
1. Add per-pixel vs per-file-mean methodology divergence to `plan/06_risks.md`.
2. Plan/15 mandate: ≥1 feature direction must demonstrably improve Non-STEC class density (e.g., aromatic-AA-anchored features that Stage 3 showed Non-STEC slightly enriched on).
3. Stage 7 complete. Stage 4 (Lorentzian peak-shift) and Stage 8 (MCR-ALS) still queued.
